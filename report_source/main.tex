...
From the video \href{https://design-research.ch/explainer-video}{\textit{How Atopical Interfaces work}}, Micheal Helke:
...
\begin{figure}[htb]
\includegraphics[width=\columnwidth]{figures/phone_drawing.png}
\caption{Examples of atopical interface usage (Micheal Helke's video)}
...
We can easily detect a firmer press of one finger this way. It has been first done by Apple with \href{https://en.wikipedia.org/wiki/Force_Touch}{3D Touch}, and by Google \textit{firm press}. The latter is briefly explained in \href{https://research.google/blog/sensing-force-based-gestures-on-the-pixel-4/}{this blog post}. In both cases the firm press is not used for a specific action, but to speed up a long press action. It makes it hard to really experience how it would feel like.
...
The goal is therefore to train a neural network on the usage data of the prototype, so that it can predict in real time if a finger is intentionally pressed. All the code for this project can be find on GitHub ()
...
Other factors can add noise and uncertainties to the intensities :
\begin{itemize}
  \item A change in handling of the prototype can suddenly increase the discharge intensity of a finger, without it pressing harder or lighter. That's because it's contact area can change whether how it's laid on the prototype. A finger laid on its side doesn't have as much contact area as a one on its tip.
  \item Fingers don't all have the same size. For instance the thumb is bigger than the pinky, so there's a big difference in discharge intensity between fingers, even at equal pressure.
  \item Similarly, fingers don't all have the same softness. For instance, the thumb is softer than the pinky. The intensity can therefore vary greatly for some fingers, and little for others.
  \item Fingers and handlings characteristics vary between users.
\end{itemize}
...
As we can see the performance increases quickly after 5 frames, but stabilizes right after. Taking too much frames is not going to improve performance, but rather make the model very big. As we want to keep it light, 10 frames seems to be a good compromise and will be used for the following models. How the performance is evaluated is explained in the following section.
...
Frellsen, the teacher of the course, for accepting my project and giving me some pointers to get started.
...
\item The tools were used to generate parts of codes, for instance neural network implementations, after describing the architecture 
\item[At what stage(s) of the process did you use the tool(s)?]
\item Mostly during the implementation part, but sometimes to suggest improvements on architectures, which were less about coding and more about "thinking"
\item[How did you use or incorporate the generated output?]
\item Either via code completion or agent mode in VSCode, which directly writes code in the editor
\end{description}
