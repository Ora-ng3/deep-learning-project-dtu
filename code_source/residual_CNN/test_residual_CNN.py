...
MODEL_PATH = CURRENT_DIR / "residual_cnn.keras"
...
def plot_worst_windows(X_val_frames, y_val_frames, val_indices, model, fp_indices, fn_indices, window_size=WINDOW_SIZE):
    ...
    def plot_indices(indices, title_prefix):
        for i, idx in enumerate(indices[:5]): # Take first 5
            ...
            # Reshape for Residual CNN: (window_size, 6) -> (6, window_size, 1)
...
def main():
    ...
    for b in range(num_batches):
        ...
        for idx in batch_indices:
            ...
            # Reshape X for Residual CNN: (window_size, 6) -> (6, window_size, 1)
    ...
    plt.title('Confusion Matrix (Residual CNN) - Tolerant')
    plt.ylabel('True Label')
    plt.xlabel('Predicted Label')
    plt.savefig(OUTPUT_DIR / "confusion_matrix_residual_cnn.png")
